...
## 2025-02-23 - Focusable Disabled Buttons
**Learning:** Native `disabled` attributes on `<button>` elements remove them from the document tab order. This is problematic for accessibility when the button relies on a `title` or `aria-label` attribute to explain *why* it is disabled (e.g., "Settings have changed. Generate XML first"). Keyboard-only and screen reader users cannot focus the button to perceive this critical contextual information.
**Action:** Instead of `disabled`, use `aria-disabled="true"` combined with identical CSS styling (e.g., `opacity: 0.5`, `cursor: not-allowed`) to keep the button focusable but visually inactive. Ensure the JavaScript `on:click` handler manually prevents the action (e.g., `e.preventDefault()`) when the button is logically disabled.
...
## 2024-07-06 - Avoid Dead-end Empty Forms
**Learning:** UX Design Pattern: Leaving a finite-option form visible when all its options are exhausted (e.g., all available items have been assigned) creates a dead-end interaction. Conditionally hide the exhausted form and replace it with an explicit instructional or success state message. When doing so, ensure keyboard focus is programmatically shifted to the new message container if the previously focused element is removed from the DOM.
**Action:** Apply dead end form avoidance pattern.
