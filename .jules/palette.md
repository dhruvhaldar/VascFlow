...
## 2024-05-18 - Hide Exhausted Forms

**Learning:** Leaving a finite-option form visible when all its options are exhausted creates a dead-end interaction, leading to user confusion and bad keyboard trapping. When conditionally hiding an exhausted form, it's critical to manage focus back to an explicit success or informational message so that keyboard users are not left on a removed element.
**Action:** Conditionally hide forms when their finite options are fully utilized, render an explicit success/informational message, and immediately transition focus to that message wrapper.
