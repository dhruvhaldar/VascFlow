...
## 2024-05-19 - Added interactive hover and inline validation states to dark-mode custom form components
**Learning:** When building custom dark-mode form components (like inputs and action buttons), relying solely on structural CSS leaves them feeling lifeless and unresponsive. Sighted users rely heavily on subtle `:hover` transitions to confirm an element is interactive. Furthermore, when auto-saving inputs to state without traditional form submissions, the absence of immediate inline visual feedback for invalid data (e.g. via `:invalid`) makes it difficult for users to diagnose why a process might fail or stall.
**Action:** Always include explicit `:hover:not(:disabled)` state transitions for custom inputs and action buttons to ensure clear visual affordances, and apply explicit `:invalid` CSS rules to `required` inputs to offer instantaneous validation feedback.
...
## 2024-08-10 - [Tooltips on Hidden Inputs in Custom Dropzones]
**Learning:** When building custom interactive drag-and-drop zones, applying a `title` tooltip to the native `<input type="file">` that is visually hidden (e.g., using `.sr-only`) makes the tooltip completely inaccessible to sighted mouse users. They cannot hover over a hidden element to see constraints or helper text. Furthermore, if the outer custom container is focusable (`tabindex="0"`) but lacks `aria-describedby` linking to the instructional text, screen reader users miss out on crucial context when tabbing to the region.
**Action:** Always place the `title` attribute on the outer, visible interactive container instead of the hidden input. Additionally, apply an `id` to the helper text/drop hint element and explicitly link it to the outer container using `aria-describedby` to ensure screen readers announce the constraints upon focus.
...
## 2024-08-27 - Use widget roles for interactive dropzones
**Learning:** When building custom drag-and-drop zones that are also click targets (focusable via `tabindex="0"` and responsive to Enter/Space), applying a landmark role like `region` or a static role like `group` is an accessibility anti-pattern. Landmarks are for structural page navigation and shouldn’t be interactive widgets. Furthermore, `aria-disabled` is not supported on `region` or `group` roles.
**Action:** Assign a widget role, such as `role="button"`, to the custom dropzone container. This accurately communicates its interactive nature to screen readers and allows valid use of state attributes like `aria-disabled`.
