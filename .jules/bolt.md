...
## 2025-04-24 - [Reuse vtk.js Rendering Pipeline]
**Learning:** Destroying and recreating `vtkActor`, `vtkMapper`, and `vtkReader` instances on every file load forces WebGL to constantly discard and reallocate heavy GPU buffers. This leads to memory fragmentation, garbage collection pauses, and noticeable mesh switching latency.
**Action:** When creating components that repeatedly load and visualize new data using `vtk.js` (like a mesh viewer), instantiate the rendering pipeline (`Reader -> Mapper -> Actor`) only once. To load new data, reuse the existing pipeline by simply updating the reader (e.g., `reader.setUrl(...)` and `reader.loadData()`). The mapper will automatically detect the updated data and efficiently refresh the WebGL buffers.
