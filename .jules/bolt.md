...
## 2024-05-19 - [Fix Blob URL Memory Leak in Svelte Components]
**Learning:** When generating temporary object URLs (like `URL.createObjectURL(file)`) conditionally based on the current upload's state (e.g., only for `.vtp` files), trapping the cleanup logic (`URL.revokeObjectURL`) inside that same conditional block causes severe memory leaks. If a user uploads a `.vtp` (creating a massive Blob in memory) and subsequently uploads a `.vtu` (which skips Blob creation), the conditional cleanup never runs, and the old Blob remains in memory indefinitely.
**Action:** Always extract `URL.revokeObjectURL` cleanups out of conditional branches specific to the *new* state and run them unconditionally when tearing down or replacing the *old* state in component lifecycles or state updates.
