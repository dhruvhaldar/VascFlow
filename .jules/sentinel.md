...
## 2025-03-24 - Explicitly disable deprecated XSS Auditor
**Vulnerability:** The application was missing the `X-XSS-Protection: 0` header. While older security guidelines recommended setting this to `1; mode=block`, the built-in XSS Auditor in legacy browsers has been deprecated because attackers could manipulate its behavior to introduce client-side vulnerabilities and extract information.
**Learning:** The modern security standard endorsed by OWASP explicitly disables the legacy auditor by setting `X-XSS-Protection: 0` and relies entirely on a strong Content Security Policy (CSP) to mitigate XSS attacks instead.
**Prevention:** Always include `X-XSS-Protection: 0` in the default security headers for the application to prevent legacy browsers from executing potentially vulnerable XSS mitigation logic.
