...
## 2025-03-25 - Security Enhancement: Explicitly disable legacy XSS auditor
...
## 2025-03-24 - Fix Disk Exhaustion DoS via Chunked Uploads
**Vulnerability:** The HTTP middleware allowed `Transfer-Encoding: chunked` requests for the `/process_mesh` file upload endpoint. Because Starlette/FastAPI automatically spools incoming request bodies (like `UploadFile`) to disk before the route handler is invoked, an attacker could send an infinite chunked stream. This would bypass the middleware's `Content-Length` limits and exhaust the server's disk space before the application-layer size validation (`save_upload_file`) could execute.
**Learning:** Application-layer streaming size limits are ineffective if the underlying web framework eagerly spools the entire payload to disk or memory beforehand. Defenses must be enforced at the lowest possible HTTP level (e.g., middleware or proxy).
**Prevention:** Strictly reject `Transfer-Encoding: chunked` in the HTTP middleware for endpoints that rely on framework-level parsing (like `UploadFile` or form data), unless you have configured the ASGI server itself (e.g., Uvicorn) to enforce a hard maximum request body size.
