...
## 2024-06-10 - Pydantic Memory Exhaustion (DoS) via Transfer-Encoding: chunked Bypass
**Vulnerability:** A `Content-Length` limiting middleware used to prevent massive JSON payloads from reaching Pydantic was successfully bypassed using `Transfer-Encoding: chunked`, which causes the client to omit the `Content-Length` header.
**Learning:** Pydantic in FastAPI validates JSON payload limits *after* loading the entire parsed body into memory. Enforcing limits via a middleware `Content-Length` check is a necessary defense-in-depth, but it is fundamentally flawed against chunked HTTP streams. Attackers can upload an unbounded, multi-gigabyte JSON payload chunk-by-chunk, fully bypassing the header check and causing the application to crash due to OOM.
**Prevention:** Always explicitly check for and reject `Transfer-Encoding: chunked` requests (e.g. `411 Length Required`) on JSON-based APIs to ensure the payload length is strictly known and safely limited prior to memory allocation.
