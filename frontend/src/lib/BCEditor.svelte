<script>
    ...
    async function addBC() {
        ...
        await tick();
        if (usedFaceNames.size >= $meshMetadata.faces.length && successMessageElement) {
...
<div class="bc-editor" role="region" aria-label="Boundary Conditions">
    ...
    {#if $meshMetadata.faces.length > 0}
        {#if usedFaceNames.size >= $meshMetadata.faces.length}
            <p class="empty-state" bind:this={successMessageElement} tabindex="-1">✓ All available faces have been assigned a boundary condition.</p>
        {:else}
