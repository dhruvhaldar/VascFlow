<script>
    ...
    async function addBC() {
        ...
        // Wait for DOM to update and then return focus to the success message
        // if all options are exhausted, or back to the Face select input,
...
<div class="bc-editor" role="region" aria-label="Boundary Conditions">
    ...
    {#if $meshMetadata.faces.length > 0}
        ...
        {:else}
            <div bind:this={successMessageElement} tabindex="-1" style="outline: none;">
                <p class="empty-state">✓ All available faces have been assigned boundary conditions.</p>
            </div>
        {/if}
