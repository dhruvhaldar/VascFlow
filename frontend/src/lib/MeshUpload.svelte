...
>
    <h3>Mesh Upload</h3>
    <input type="file" bind:this={fileInput} on:change={handleFileSelect} accept=".vtu,.vtp,.vtk" aria-label="Upload Mesh File" title={loading ? "Processing upload, please wait..." : "Choose a mesh file to upload"} disabled={loading} aria-invalid={error ? 'true' : undefined} aria-describedby={error ? 'mesh-upload-error' : undefined} />
    {#if !loading}
